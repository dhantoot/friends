...
interface Card2DivInvertedProps {
  className?: string;
  style?: React.CSSProperties;
}

const Card2DivInverted: React.FC<Card2DivInvertedProps> = ({ className, style }) => {
  return (
    <div className={`relative w-full aspect-[366/170] px-[2.6%] ${className}`} style={style}>
      {/* Background Layer with Vertically Flipped Milky Card Gradient */}
      <div className="absolute inset-0 overflow-visible scale-y-[-1]">
        <svg className="w-full h-full" viewBox="0 0 366 170" fill="none" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <linearGradient id="card2InvertedGradient" x1="0%" y1="0%" x2="0%" y2="100%">
          ...
          <path
            ...
            fill="url(#card2InvertedGradient)"
      ...
      {/* Content Area - Not flipped so text stays readable */}
...
export default Card2DivInverted;
